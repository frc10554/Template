...
class PhoenixOdometryThread private constructor() : Thread() {
    ...
    fun registerSignal(signal: StatusSignal<Angle?>?): Queue<Double?> {
        ...
        try {
            ...
            phoenixSignals = newSignals.filterNotNull().toTypedArray()
    ...
    companion object {
        private val isCANFD = TunerConstants.kCANBus.isNetworkFD
        private var _instance: PhoenixOdometryThread? = null

        fun getInstance(): PhoenixOdometryThread {
            if (_instance == null) {
                _instance = PhoenixOdometryThread()
            }
            return _instance!!
        }
    }
